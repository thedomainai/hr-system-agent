...
**設計方針:**
- MVP: ユーザーは「入力」と「承認/改善リクエスト」のみ
- エージェントは入力情報から自動で制度案を生成
- 将来拡張: 改善リクエスト時や詳細調整時に対話型プロセスを追加可能
...
| IndustryResearcher | 業界ベンチマーク調査 | 企業規模 | 市場データ |
...
| HITL-002 | 求める人材像生成後 | 承認/改善リクエスト | 72時間 |
| HITL-003 | 等級制度案生成後 | 承認/改善リクエスト | 72時間 |
| HITL-004 | 評価制度案生成後 | 承認/改善リクエスト | 72時間 |
| HITL-005 | 報酬制度案生成後 | 承認/改善リクエスト | 72時間 |
...
| POST | /api/v1/reviews/{id}/reject | 改善リクエスト |
...
| HITL承認 | 各制度案（7ゲート）の承認/改善リクエストができる |
...
| G2: 企業特性カスタマイズ | 企業規模・経営理念に応じた提案が生成される |
